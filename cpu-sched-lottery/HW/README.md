1.  
```sh
jacky@MacBook-Pro-2 cpu-sched-lottery % ./lottery.py -j 3 -s 1 -c
ARG jlist 
ARG jobs 3
ARG maxlen 10
ARG maxticket 100
...
jacky@MacBook-Pro-2 cpu-sched-lottery % ./lottery.py -j 3 -s 2 -c
ARG jlist 
ARG jobs 3
ARG maxlen 10
ARG maxticket 100
...
jacky@MacBook-Pro-2 cpu-sched-lottery % ./lottery.py -j 3 -s 3 -c
ARG jlist 
ARG jobs 3
ARG maxlen 10
ARG maxticket 100
...
2.  
```sh
jacky@MacBook-Pro-2 cpu-sched-lottery % ./lottery.py -l 10:1,10:100 -c
...
```
job0幾乎執行不到，這種不平衡造成飢餓現象。  
3. 這是比較公平的方式  
4. quantum size變長，導致整個工作時常變短，會增加不公平性。  
